...
public class JiGuangProxyPlugin extends TrialProxyPlugin {
    ...
    private static List<LoginParam> loginParams = new ArrayList<>();
...
//            request.setCookie(StringUtils.isEmpty(redisCookie) ? cookie : redisCookie);
    ...
    public void login(LoginParam param) {
        ...
        for (int i = 0; i < getLoginRetryCount(); i++) {
            ...
            if (PatternUtils.match(response.getRespText(), "\\\\\\\"ret\\\\\\\":0")) {
                ...
                cookie = client.getContextCookies();
                return;
            } else {
                break;
            }
